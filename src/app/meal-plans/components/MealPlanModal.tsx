          ...
          <div className="relative bg-gradient-to-r from-orange-500 to-orange-600 p-8 text-white">
            ...
            <div className="flex items-start justify-between pr-16">
              ...
              <motion.div 
                ...
                transition={{ delay: 0.3 }}
              >
                <div className="text-3xl font-bold mb-2">Rp.{mealPlan.price.toString()}</div>
                <div className="flex gap-2">
                  ...
                  >
                    <Heart className={`w-5 h-5 ${isLiked ? 'fill-red-300 text-red-300' : 'text-white'}`} />
                  </motion.button>
                  <motion.button
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  <Card className="border-orange-200 hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="space-y-3 pt-4">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-orange-600">{mealPlan.meals.length}</span>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    {selectedMealType && (
                      ...
                      >
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                          {groupedMeals[selectedMealType]?.map((meal, index) => (
                            <motion.div
                              key={meal.id}
                              initial={{ opacity: 0, y: 20 }}
                              animate={{ opacity: 1, y: 0 }}
                            ...
                            >
                              <Card className="bg-gradient-to-br from-orange-50 to-amber-50 border-orange-200 hover:shadow-lg transition-all hover:scale-105">
                                <CardContent className="p-5">
                                  <h5 className="font-semibold text-gray-900 mb-2 text-lg">{meal.name}</h5>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 text-xs">
                                      ...
                                      {meal.protein && (
                                        <div className="flex items-center justify-between bg-white/60 p-2 rounded-lg">
                                          ...
                                          <span className="font-semibold text-orange-600">{meal.protein.toString()}g</span>
                                        </div>
                                      )}
                                      {meal.carbs && (
                                        <div className="flex items-center justify-between bg-white/60 p-2 rounded-lg">
                                          <span className="text-gray-600">Carbs:</span>
                                          <span className="font-semibold text-orange-600">{meal.carbs.toString()}g</span>
                                        </div>
                                      )}
                                      {meal.fats && (
                                        <div className="flex items-center justify-between bg-white/60 p-2 rounded-lg">
                                          <span className="text-gray-600">Fats:</span>
                                          <span className="font-semibold text-orange-600">{meal.fats.toString()}g</span>
                                        </div>
                                      )}
                                    </div>
                                  )}
              ...
              <TabsContent value="nutrition" className="space-y-6">
                ...
                >
                  ...
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">Nutrition Information</h3>
                  <p className="text-gray-600">Detailed nutrition facts and analysis coming soon!</p>
                </motion.div>
              </TabsContent>
            </Tabs>
          </div>
          ...
          >
            ...
            <div className="flex gap-3">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="outline" onClick={onClose} className="border-orange-300 text-orange-600 hover:bg-orange-50">
                  Close
                </Button>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
