...
export default function SubscriptionForm() {
  ...
  const getSelectedPlan = () => {
    return mealPlans.find(plan => plan.category.toLowerCase() === formData.planCategory.toLowerCase());
  };

  const getMealsByType = (type: string) => {
    const selectedPlan = getSelectedPlan();
    return selectedPlan?.meals.filter(meal => meal.mealType.toLowerCase() === type.toLowerCase()) || [];
  };
...
const canProceedToStep2 = formData.name.trim() && phoneIsValid;
  ...
  const canProceedToStep4 = formData.subscriptionType === 'package' ||
  (validateDeliveryDays(formData.days) && validateMealSelection(formData.selectedMeals, formData.subscriptionType));
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      ...
      {step === 1 && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-orange-600 mb-6">Personal Information</h2>
      ...
      {step === 2 && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-orange-600 mb-6">Select Your Plan Category</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {MEAL_CATEGORIES.map((category) => {
              const plan = mealPlans.find(p => p.category.toLowerCase() === category.toLowerCase());
              return (
                <div
                  key={category}
                ...
                >
                  ...
                  <p className="text-gray-600 mb-4">{plan?.description || 'Perfect for your lifestyle'}</p>
                  <p className="text-orange-600 font-bold">Rp {formatPriceWithoutSymbol(plan?.price || 0)}/week</p>
                  {plan?.features && (
      ...
      {step === 3 && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-orange-600 mb-6">Customize Your Plan</h2>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-sm text-gray-600">Choose your own meals for each type</p>
              </div>
            </div>
          </div>
          ...
          {/* Delivery Days */}
            {formData.subscriptionType === 'custom' && (
            <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Select Delivery Days</h3>
              ...
              {['breakfast', 'lunch', 'dinner'].map((mealType) => (
                <div key={mealType}>
                  <h3 className="text-lg font-semibold text-gray-800 mb-3 capitalize">
                    Select {mealType} (Rp {formatPriceWithoutSymbol(MEAL_PRICES[mealType as keyof typeof MEAL_PRICES])})
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {getMealsByType(mealType).map((meal) => (
                      <div
                        key={meal.id}
                        onClick={() => handleMealSelection(mealType as 'breakfast' | 'lunch' | 'dinner', meal.id)}
                        className={`p-4 border-2 rounded-lg cursor-pointer ${
                          formData.selectedMeals[mealType as keyof typeof formData.selectedMeals] === meal.id
                            ? 'border-orange-500 bg-orange-50'
                            : 'border-gray-200'
                        }`}
                      >
                        ...
                        {meal.calories && (
                          <p className="text-sm text-orange-600 mt-2">{meal.calories} calories</p>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      {step === 4 && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-orange-600 mb-6">Review Your Subscription</h2>
          ...
          <div className="bg-gray-50 p-6 rounded-lg space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 capitalize">Category: {formData.planCategory}</p>
                <p className="text-gray-600 capitalize">Type: {formData.subscriptionType} Plan</p>
              </div>
            </div>
            ...
            <div>
                ...
                {formData.subscriptionType === 'package' ? (
                    <>
                    <p className="text-gray-600">Days: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday</p>
                    <p className="text-gray-600">Total Days per Week: 7</p>
                    </>
                ) : (
                    <>
                    <p className="text-gray-600">Days: {formData.days.join(', ')}</p>
                    <p className="text-gray-600">Total Days per Week: {formData.days.length}</p>
                    </>
                )}
            </div>
            ...
            {formData.allergies && (
              <div>
                <h3 className="font-semibold text-gray-800">Allergies/Restrictions</h3>
            ...
            <p className="text-gray-600 flex justify-between">
                ...
                <span>Rp {formatPriceWithoutSymbol(getSelectedPlan()?.price || 0)}</span>
            </p>
            ) : (
                <>
                {formData.days.map((day) => (
                    ...
                    <ul className="ml-4 list-disc">
                        {['breakfast', 'lunch', 'dinner'].map((mealType) => {
                        const mealId = formData.selectedMeals[mealType as keyof typeof formData.selectedMeals];
                        if (!mealId) return null;
                        const meal = getMealsByType(mealType).find(m => m.id === mealId);
                        return (
                            <li key={mealId}>
                            {meal?.name || 'Meal'} - Rp {formatPriceWithoutSymbol(MEAL_PRICES[mealType as keyof typeof MEAL_PRICES])}
                            </li>
                        );
                        })}
                    </ul>
            ...
            <div className="flex justify-between text-xl font-bold pt-2 border-t mt-2">
                ...
                <span className="text-orange-600">Rp {formatPriceWithoutSymbol(calculatePrice())}</span>
            </div>
            </div>
